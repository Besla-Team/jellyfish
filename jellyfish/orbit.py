...
def orbit(path, snap_name, initial_snap, final_snap, Nhost_particles, delta, lmc=False, disk=False):
    ...
    for i in range(initial_snap, final_snap+1):
        ...
        ## computing COM
        if lmc==True:
            host_particles = host_sat_particles(pids, Nhost_particles, 0, xyz, vxyz) 
            host_xyz = host_particles[0]
            host_vxyz = host_particles[1]

            sat_particles = host_sat_particles(pids, Nhost_particles, 1, xyz, vxyz)
            sat_xyz = sat_particles[0]
            sat_vxyz = sat_particles[1]
            ...
            sat_vcm[i-initial_snap] =  velocities_com(sat_rcm[i-initial_snap], sat_xyz, sat_vxyz, 5)
